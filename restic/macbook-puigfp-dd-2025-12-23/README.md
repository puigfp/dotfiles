...
- schedule CRON task: `dotenvx run -- resticprofile schedule`
  ...
  - edit the line to prefix the call with `dotenvx -- run`:
    ```
    2 */6 * * * cd /Users/francisco/dev/dotfiles/restic/2025-12-23-puigfp-dd && dotenvx run -- /opt/homebrew/bin/resticprofile --no-ansi --config profiles.yaml run-schedule backup@default
    ```
